...
int
Triangle::onSideOfPlane(Axis axis, float d) const
{
	float f;
	// We test the bounding box!
	switch (axis) {
		case axisX:
			f = pos.x;
			if (pos2.x > f) f = pos2.x;
			if (pos3.x > f) f = pos3.x;
			if (f < d) return -1;
			f = pos.x;
			if (pos2.x < f) f = pos2.x;
			if (pos3.x < f) f = pos3.x;
		...
		case axisY:
			f = pos.y;
			if (pos2.y > f) f = pos2.y;
			if (pos3.y > f) f = pos3.y;
			if (f < d) return -1;
			f = pos.y;
			if (pos2.y < f) f = pos2.y;
			if (pos3.y < f) f = pos3.y;
		...
		case axisZ:
			f = pos.z;
			if (pos2.z > f) f = pos2.z;
			if (pos3.z > f) f = pos3.z;
			if (f < d) return -1;
			f = pos.z;
			if (pos2.z < f) f = pos2.z;
			if (pos3.z < f) f = pos3.z;
